...
object MultithreadingRecap extends App {
  ...
  val aThread = new Thread(() => println("I1m running in parallel"))
  aThread.start()
  aThread.join()
  ...
  future.onComplete {
    ...
    case Success(_)  => println("Meaning of life is empty")
    case Failure(_)  => println("Something happenedwith the meaning of life!")
  }

  val aProcessedFuture = future.map(_ + 1) // Future with 42
  val aFlatFuture = future.flatMap(value => Future(value + 2)) // Future with 42
